...
async function renderAv2Resumen() {
  ...
  c.innerHTML = `
    ...
    <!-- Métricas operativas (en vivo). Las cifras de dinero son ESTIMADAS; la fuente exacta es Stripe. -->
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 mb-2">
      ...
      ${tile('💰', '#ecfdf5', '#059669', 'Ingreso estimado / mes', _av2Sek(m.revenueMonth), 'estimado · cifra exacta en Stripe')}
    </div>
    <div class="text-[11px] text-gray-400 mb-4 px-1">💡 Las cifras de dinero son una estimación (suscripciones activas × su precio). La contabilidad real —con pagos fallidos, reembolsos, prorrateos e impuestos— está en <a href="https://dashboard.stripe.com" target="_blank" rel="noopener" class="text-swe-blue hover:underline font-semibold">Stripe</a>.</div>
  ...
  // (Gráfico de ingresos anual retirado: las cifras exactas están en Stripe.)
...
function showGrammarResult() {
  ...
  const _jmot = document.getElementById('gq-motivation-card');
...
async function initUnifiedProgress() {
  ...
  // Cross-device: sincroniza la racha (fila meta|streak). Guarda el conteo (progress_value)
  // y la fecha del último día de estudio (score = YYYYMMDD). Solo considera rachas "vivas"
  // (último día = hoy o ayer) para no inflar ni reiniciar incorrectamente. getStudyStreak()
  // hace el ajuste final (extiende si es ayer, mantiene si es hoy).
  try {
    const _di = d => d ? parseInt(String(d).replace(/-/g, ''), 10) : 0;
    const today = new Date().toISOString().slice(0, 10);
    const yest = new Date(Date.now() - 864e5).toISOString().slice(0, 10);
    const alive = d => d === today || d === yest;
    ...
    const cloudDateInt = row && row.score ? row.score : 0;
    const cloudDateStr = cloudDateInt ? `${String(cloudDateInt).slice(0, 4)}-${String(cloudDateInt).slice(4, 6)}-${String(cloudDateInt).slice(6, 8)}` : null;
    const cands = [];
    if (local && local.count && alive(local.last)) cands.push({ count: local.count, last: local.last });
    if (cloudCount && cloudDateStr && alive(cloudDateStr)) cands.push({ count: cloudCount, last: cloudDateStr });
    if (cands.length) {
      const best = cands.sort((a, b) => (b.count - a.count) || (_di(b.last) - _di(a.last)))[0];
      try { localStorage.setItem('sc_streak', JSON.stringify(best)); } catch (e) {}
      if ((best.count > cloudCount || _di(best.last) > cloudDateInt) && typeof progressMark === 'function') {
        progressMark('meta', 'streak', { status: 'in_progress', progress_value: best.count, score: _di(best.last) });
      }
    }
  } catch (e) {}
...
function getStudyStreak() {
  try {
    ...
    let changed = false;
    if (!s || !s.last) { s = { count: 1, last: t }; changed = true; }
    else if (s.last !== t) {
    ...
    // Cross-device: cuando la racha cambia (una vez al día), súbela a la nube.
    if (changed) { try { if (typeof progressMark === 'function' && window._sbSession && window._sbSession.id) progressMark('meta', 'streak', { status: 'in_progress', progress_value: s.count, score: parseInt(t.replace(/-/g, ''), 10) }); } catch (e) {} }
    return s.count || 1;
  } catch (e) { return 1; }
}
...
function renderAccesoRapido() {
  ...
  if (!el) return;
  el.innerHTML = [
    _accesoCard({ img: '/juanita/juanita-chancleta.webp', bg: '#ffd9dc', cardBg: '#fff1f2', cardBorder: '#fecdd3', title: 'Practicando con Juanita', text: 'Practica por nivel, con Juanita animándote.', badge: '', badgeCls: '', onclick: "openJuanita()" }),
