...
// ── Navigation ───────────────────────────────────────────────
const VIEW_URLS = { home: '/plataforma', login: '/alumnos', alumnos: '/', resenas: '/reseñas', 'admin-v2': '/admin' };
...
async function renderAv2Resumen() {
  ...
  c.innerHTML = `
    ...
    <div class="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100 av2-shadow mb-4">
      <div class="flex items-start justify-between mb-4 flex-wrap gap-2">
        <div>
          <div class="text-sm font-bold text-gray-800">Ingresos recurrentes</div>
          <div class="text-[26px] leading-none font-extrabold text-gray-900 mt-1">${_av2Sek(m.revenueMonth)}<span class="text-sm font-medium text-gray-400"> / mes</span></div>
        </div>
        <div class="text-right">
          <div class="text-[11px] text-gray-400">ARPU (por alumno)</div>
          <div class="text-sm font-bold text-gray-700">${_av2Sek(m.active.length ? Math.round(m.revenueMonth / m.active.length) : 0)}</div>
        </div>
      </div>
      <canvas id="av2-revenue-chart" height="80"></canvas>
      <div class="text-[11px] text-gray-400 mt-2">Estimado por alumnos activos y su fecha de alta · últimos 12 meses</div>
    </div>
  ...
  // ── Gráfico de ingresos por mes (estilo Stripe: área con degradado) ──
  try {
    ...
    if (rctx && typeof Chart !== 'undefined') {
      const serie = _av2RevenueSeries(students, (cfg && cfg.priceLegacy) || (cfg && cfg.priceNew) || 339, 12);
      const g = rctx.getContext('2d');
      const grad = g.createLinearGradient(0, 0, 0, 180);
      grad.addColorStop(0, 'rgba(0,106,167,0.22)');
      grad.addColorStop(1, 'rgba(0,106,167,0.00)');
      _av2Charts.revenue = new Chart(rctx, {
        type: 'line',
        data: { labels: serie.map(s => s.label), datasets: [{ data: serie.map(s => s.value), borderColor: '#006AA7', borderWidth: 2.5, backgroundColor: grad, fill: true, tension: 0.4, pointRadius: 0, pointHoverRadius: 5, pointHoverBackgroundColor: '#006AA7', pointHoverBorderColor: '#fff', pointHoverBorderWidth: 2 }] },
        options: {
          responsive: true, maintainAspectRatio: false,
          interaction: { intersect: false, mode: 'index' },
          plugins: { legend: { display: false }, tooltip: { backgroundColor: '#0f172a', padding: 10, cornerRadius: 10, displayColors: false, callbacks: { title: it => it[0].label, label: it => _av2Sek(it.raw) + ' · ' + serie[it.dataIndex].count + ' activos' } } },
          scales: { y: { beginAtZero: true, border: { display: false }, grid: { color: '#f1f5f9' }, ticks: { maxTicksLimit: 5, font: { size: 10 }, color: '#94a3b8', callback: v => (v >= 1000 ? (v / 1000) + 'k' : v) } }, x: { border: { display: false }, grid: { display: false }, ticks: { font: { size: 10 }, color: '#94a3b8', maxRotation: 0, autoSkipPadding: 12 } } }
        }
      });
    }
  } catch (e) {}
