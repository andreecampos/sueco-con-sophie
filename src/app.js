...
async function loadAssignedLevelFromDB() {
  ...
  try {
    ...
    if (r && r.nivel) {
      ...
      // Cross-device: si este dispositivo no tiene el resultado en localStorage, lo cacheamos
      // desde la BD para que getLastNivel() (y el anillo de avance) funcionen en cualquier PC.
      try {
        let local = null; try { local = JSON.parse(localStorage.getItem('scs_nivel_last') || 'null'); } catch (e) {}
        if (!local || !local.pct) {
          const total = r.total || 1;
          const pct = r.pct != null ? r.pct : Math.round((r.puntaje || r.correct || 0) / total * 100);
          const skills = { las: r.las || 0, hor: r.hor || 0, skriv: r.skriv || 0, tala: r.tala || 0 };
          localStorage.setItem('scs_nivel_last', JSON.stringify({ nivel: r.nivel, pct: pct, skills: skills, ts: Date.parse(r.created_at) || Date.now() }));
        }
      } catch (e) {}
    }
  } catch (e) {}
